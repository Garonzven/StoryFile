    ...
    public class CustomPackerPolicy : UnityEditor.Sprites.IPackerPolicy
    {
        ...
        public void OnGroupAtlases(BuildTarget target, UnityEditor.Sprites.PackerJob job, int[] textureImporterInstanceIDs)
        {
            ...
            foreach (int instanceID in textureImporterInstanceIDs)
            {
                ...
                int maxWidth = EditorPrefs.GetInt( CustomMenuItems.CUSTOMPP_MAX_WIDTH, 2048 );
                int maxHeight = EditorPrefs.GetInt( CustomMenuItems.CUSTOMPP_MAX_HEIGHT, 2048 );
                bool generateMipMaps = EditorPrefs.GetBool( CustomMenuItems.CUSTOMPP_MIPMAPS, true );

                TextureFormat desiredFormat;
                ColorSpace colorSpace;
                int compressionQuality;
                ti.ReadTextureImportInstructions(target, out desiredFormat, out colorSpace, out compressionQuality);
                desiredFormat = (TextureFormat) EditorPrefs.GetInt( CustomMenuItems.CUSTOMPP_TEXTURE_FORMAT + Application.platform, 
                    #if UNITY_IOS
                ...
                foreach (Sprite sprite in sprites)
                {
                    ...
                    if( sprite.rect.width > maxWidth || sprite.rect.height > maxHeight )
                    {
                        maxWidth *= 2;
                        maxHeight *= 2;
                    }
                    entry.settings.maxWidth = maxWidth;
                    entry.settings.maxHeight = maxHeight;
                    entry.settings.generateMipMaps = generateMipMaps;
